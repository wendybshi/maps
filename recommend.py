...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    #minDis= [min([distance(location, x) for x in centroids]
    #closest= [centroids[x] for x in centroids if distance(location, dis)==minDis]
    return min(centroids,key=lambda cen:distance(location, cen))
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    listOfTup= [find_predictor(user,reviewed,fn)for fn in feature_fns]
    return (max(listOfTup, key=lambda x:x[1]))[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    def restaurantRating(r):
        if r in reviewed:
            return user_rating(user, restaurant_name(r))
        else:
            return predictor(r)
    return {restaurant_name(oneRestaurant):restaurantRating(oneRestaurant) for oneRestaurant in restaurants}
    # END Question 9
